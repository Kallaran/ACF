...
(* Pour une clé k et une table t, (assoc k t) rend None si la clé k ne figure pas
   dans t et (Some v) si k est associée Ã  v dans t *)
...
(* Pour une clé k, une valeur v et une table t, (modify k v t) rend la table dans laquelle
   k est associée Ã  v. Si l'association n'existait pas elle est crée. *)
...
(* Lemme1: Si toutes les clés sont uniques dans la table t, un couple (k,v) appartient Ã  la liste t
   si et seulement quand on cherche k dans la table on trouve v. *)

(* Lemme2: Soit une table t, si on modifie la valeur associée Ã  k dans t (on lui associe une nouvelle valeur v), 
   alors si l'on cherche k dans t on obtient v. *)

(* Lemme3: Soit une table t, dans laquelle k2 n'apparait pas.
   Si on change la valeur associée Ã  k (k\<noteq>k2) dans t, alors si l'on 
   cherche k2 dans t, elle n'apparait toujours pas. *)

(* Lemme4: Soit une table t, dans laquelle k est associée Ã  la valeur v.
   Si on change la valeur associée Ã  k2 (k\<noteq>k2) dans t, alors si l'on cherche k 
   dans t, elle est toujours associée Ã  v. *) 
...
(* Lemme6: Si tous les éléments d'une liste l satisfont un prédicat p, et que e appartient 
   Ã  la liste p, alors e satisfait p. *)
